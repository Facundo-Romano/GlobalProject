const preLoadedActivities = [
    {
        name: "Hiking",
        difficulty: "3",
        duration: "2",
        season: "Winter",
        img: "https://images.unsplash.com/photo-1551632811-561732d1e306?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8M3x8aGlraW5nfGVufDB8fDB8fA%3D%3D&w=1000&q=80",
        countries: ["QAT", "ARG", "AFG", "MEX", "ALA"]
    },
    {
        name: "Surfing",
        difficulty: "4",
        duration: "1",
        season: "Summer",
        img: "https://static01.nyt.com/images/2021/08/22/books/review/Bissell/Bissell-facebookJumbo.jpg",
        countries: ["AFG", "ALA"]
    },
    {
        name: "Shopping",
        difficulty: "1",
        duration: "5",
        season: "Spring",
        img: "https://www.globalblue.com/business/images/article926851.ece/alternates/LANDSCAPE2_970/TFS_Article_2019_970x643.jpg",
        countries: ["QAT", "ARG", "AFG", "MEX"]
    },
    {
        name: "Painting",
        difficulty: "1",
        duration: "3",
        season: "Winter",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQKPgfn8s_B56kGmZdEKBep7la98dZ2xr6ScQ&usqp=CAU",
        countries: ["QAT", "ARG", "AFG", "MEX", "ALA"]
    },
    {
        name: "Sightseeing",
        difficulty: "2",
        duration: "4",
        season: "Summer",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS0AWqnzdq4nWN20xA8ln4wLj4va4RWmGiERQ&usqp=CAU",
        countries: ["ARG", "QAT", "MEX", "ALA", "AFG"]
    },
    {
        name: "Party",
        difficulty: "3",
        duration: "5",
        season: "Fall",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRC8bY49xvIdyBSrIP-0NHHyo0gxYcyZAj76A&usqp=CAU",
        countries: ["ARG", "QAT", "MEX", "ALA", "AFG", "AND"]
    },
    {
        name: "Restaurant",
        difficulty: "1",
        duration: "1",
        season: "Spring",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAIUGEYzl4iSH-KrDEzxkWRiLu4CTn1SreRw&usqp=CAU",
        countries: ["ARG", "QAT", "MEX", "ALA", "AFG", "AND", "BHS"]
    },
    {
        name: "Adventure travel",
        difficulty: "5",
        duration: "5",
        season: "Summer",
        img: "https://travelsud.com/wp-content/uploads/2020/01/Adventure-Travel-Agents.jpg",
        countries: ["ARG", "QAT", "MEX", "ALA", "AFG", "AND"]
    },
    {
        name: "Climbing",
        difficulty: "4",
        duration: "1",
        season: "Winter",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRbPcaf-JIKUzFf91og_IcBa1EoJl_l4GkZmA&usqp=CAU",
        countries: ["ARG", "QAT", "MEX", "ALA", "AFG", "AND", "BHS"]
    },
    {
        name: "Boat tour",
        difficulty: "3",
        duration: "3",
        season: "Summer",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQOFqTN0qRlJ3WKPNO_KEfsM14oRMk2ehSWig&usqp=CAU",
        countries: ["ARG", "QAT", "MEX", "ALA", "AFG", "AND", "BHS", "BRB"]
    } 
];

module.exports = {
    preLoadedActivities
};